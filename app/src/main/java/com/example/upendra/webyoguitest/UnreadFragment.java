...
public class UnreadFragment extends Fragment {
    ...
    private JSONArray jsonUnread = null;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        mailData = getArguments().getString("UnreadMails");
    ...
    private List<MailListItem> getData(String mailsData) {
        ...
        try {
            jsonUnread = new JSONArray(mailsData);
        } catch (JSONException e) {
            e.printStackTrace();
        }
        JSONObject jsonObject;
        for (int i = 0; i < jsonUnread.length(); i++) {
            try {
                jsonObject = jsonUnread.getJSONObject(i);
